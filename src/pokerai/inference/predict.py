...
def predict_action(
    ...
    if bos is None:
        raise ValueError("Tokenizer is missing bos_token_id")
    input_ids = [bos] + tokenizer(prompt, add_special_tokens=False)["input_ids"]
    budget = N_POSITIONS - max_new_tokens
    if budget < 1:
        raise ValueError(
            f"max_new_tokens={max_new_tokens} leaves no room under N_POSITIONS={N_POSITIONS}"
        )
